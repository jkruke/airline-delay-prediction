...
#print("DEBUGGING: Remove these fake dates")
...
print("Saving weather reports to all_weathers.csv")
all_weathers_dfs.to_csv("all_weathers.csv")
print(">Done")
...
delays = sqldf(f'''SELECT *
				FROM delays AS d LEFT OUTER JOIN  
					(SELECT iata, date,
							...
							wind_gusts_10m AS arr_wind_gusts_10m FROM all_weathers_dfs) AS a
...
delays = sqldf(f'''SELECT *
				FROM delays AS d LEFT OUTER JOIN  
					(SELECT iata, date,
							...
							wind_gusts_10m AS dep_wind_gusts_10m FROM all_weathers_dfs) AS dep
				ON d.dep_iata = dep.iata 
					AND unixepoch(a.date) <= unixepoch(dep.dep_time_utc) 
